 ...
 */
@Autonomous(name="Full Autonomous Red Side",group="Auton")
@Disabled
public class Red_Auton_v5 extends OpMode {
    ...
    static final double WHEEL_DIAMETER_INCHES = 5.2;
    ...
    static final double REVERSE_DIST                                = 9;                // How far the bot reverses after pushing the glyphs in
    static final double REVERSE_TO_PIT_DIST                         = 8;
    static final double PARKING_DIST                                = -2;
    static final double FAST_DRIVE_SPEED                            = .6;
    ...
    static final double RIGHT_COL_DIST_RED                          = -(23 - 5);
    ...
    static final double ALIGN_ARM_DETECT                            = .70;
    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case 0:
                relicArm.setPosition(.45);
                alignArm.setPosition(.40);
                ...
                if(runtime.time() < .25) {
                    ...
                    if (blockCount != 2) {
                        if (!block) {
                            ...
                            if (suckStart + suckTime.time() < 1.5) {
                                IMUdrive(.2, boxAngle);
                            } else if (suckStart + suckTime.time() < 2) {
                                if (i == 0){
                            ...
                            if (bDist.getDistance(DistanceUnit.CM) < 100){
                                sucked = true;
                            }

                            // If the block is not sucked up within the given delay
                            // The bot jostles the stack by turning in place
                            // If still no block is sucked up, the alg returns to the original movmement sequence
            ...
            case 7:
                ...
                if (encoderIMUdrive(-(encoderCount/COUNTS_PER_INCH), -SLOW_DRIVE_SPEED, boxAngle)) {
                    fastNextState();
                }
                break;
