 ...
 */
@Autonomous(name="Full Autonomous Blue Side Cleaned Code",group="Auton")
@Disabled
public class Blue_Jankers_Auton_v6 extends OpMode {
    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case 4:
                ...
                if (this.PIDdrive(REVERSE_TO_PIT_DIST, FAST_DRIVE_SPEED)) {
                    this.nextState();
                }
                break;
            ...
            case 5:
                ...
                if(runtime.time() < .25) {
                    ...
                    RobotLog.ii("INFO", "Min Voltage: %s", minV);
                    RobotLog.ii("INFO", "Max Voltage: %s", maxV);
                } else if (runtime.time() < 5) {
                    if (blockCount != 2) {
                        RobotLog.ii("STATE", "Sucking up Glyphs");
                        RobotLog.ii("INFO", "Voltage Difference: %s", minV - vs.getVoltage());
                        rearLeftMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
                        frontLeftMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
                        rearRightMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
                        frontRightMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);

                        if (minV - vs.getVoltage() > 1 && !block) {
                            RobotLog.ii("STATE", "Encountered Voltage Drop/Glyph");
                            checkDelay = runtime.time() + 1.5;
                            suckStart = runtime.time();
                            dropV = vs.getVoltage();
                            block = true;
                        }
                        ...
                        if (!block) {
                            ...
                            RobotLog.ii("INFO", "Voltage During Glpyh", vs.getVoltage());
                            frontRightMotor.setPower(0);
                            frontLeftMotor.setPower(0);
                            rearRightMotor.setPower(0);
                            rearLeftMotor.setPower(0);
                            if (minV - vs.getVoltage() < 1 && runtime.time() > checkDelay) {
                                RobotLog.ii("STATE", "Sucked in Glyph");
                                RobotLog.ii("INFO", "Block Count: %s", blockCount);
                                blockCount++;
                                block = false;
                            }
                        }
            ...
            case 12:
                ...
                if(currentCol.equals("Center")){
                    if (Left || Right){
                        ...
                        RobotLog.ii("INFO", "At Center going to Left");
                    ...
                    if (!Center){
                        RobotLog.ii("INFO", "At Left going to Center");
                    ...
                    if (!Center){
                        RobotLog.ii("INFO", "At Right going to Center");
            ...
            case 14:
                RobotLog.ii("STATE", "Reversing from Cryptobox");
                if (PIDdrive(1, SLOW_DRIVE_SPEED)){
