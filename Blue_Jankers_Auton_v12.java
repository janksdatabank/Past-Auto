 ...
 */
@Autonomous(name="Full Autonomous Blue Side Speed Optimized",group="Auton")
@Disabled
public class Blue_Jankers_Auton_v12 extends OpMode {
    ...
    static final double I_C_TURN = 0.0000825;
    ...
    static final double GLYPH_PUSH_DIST                             = -4.5;            // How far the bot pushes the glyphs forward after dumping
    ...
    static final double CENTER_COL_DIST_BLUE                        = 23;               // The dist the bot runs off platform to align w/ center col (Blue)
    static final double RIGHT_COL_DIST_BLUE                         = 23 + 6;
    ...
    static final double GLYPH_CLAMP_UNLOCK                          = .475;
        ...
        switch (state) {
            ...
            case 5:
                ...
                if(runtime.time() < .125) {
                    ...
                    if (blockCount != 2) {
                        if (!block) {
                            if (minV - vs.getVoltage() > 1.2) {
                                RobotLog.ii("STATE", "Encountered Voltage Drop/Glyph");
                                checkDelay = runtime.time() + .5;
                                block = true;
                            }

                            if (suckTime.time() < .5) {
                                IMUdrive(.5, boxAngle);
                            } else if (suckTime.time() < 1) {
                                if (i == 0){
                            ...
                            if (minV - vs.getVoltage() < .3 && runtime.time() > checkDelay) {
            ...
            case 7:
                ...
                if (encoderIMUdrive(-(encoderCount/COUNTS_PER_INCH), -FAST_DRIVE_SPEED, boxAngle)) {
                    fastNextState();
                }
                break;
            ...
            case 8:
                if(!atWall) {
                    ...
                    if (digitalTouch.getState() == true) {
                        ...
                        // Comment out this if statement to have the bot dumpe the second pair of glyphs
                        if (columns ==2){
                            goToState(14);
                        }
                    }
