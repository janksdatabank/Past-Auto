 ...
 */
@Autonomous(name="Returning to OG Pos with Encoders",group="Auton")
@Disabled
public class Encoder_Test_v3 extends OpMode {
    ...
    public void loop() {
        ...
        //telemetry.addData("Original Position: ", encoderCount);
        //telemetry.addData("Reverse Distance: ", (Math.abs(encoderCount)/COUNTS_PER_INCH));
        ...
        switch (state) {
            ...
            case 0:
                ...
                if (runtime.time() < .5) {
                    ...
                    eCount_FL = frontLeftMotor.getCurrentPosition();
                    eCount_FR = frontRightMotor.getCurrentPosition();
                    eCount_RL = rearLeftMotor.getCurrentPosition();
                    eCount_RR = rearRightMotor.getCurrentPosition();
            ...
            case 2:
                if(PIDdrive(-(Math.abs(eCount_FL)/COUNTS_PER_INCH), -(Math.abs(eCount_FR)/COUNTS_PER_INCH), -(Math.abs(eCount_RL)/COUNTS_PER_INCH), -(Math.abs(eCount_RR)/COUNTS_PER_INCH), -.3)){
                    this.nextState();
                }
    ...
    public boolean PIDdrive(double dist_FL, double dist_FR, double dist_RL, double dist_RR, double speed) {

        int counts_FL = (int) (COUNTS_PER_INCH * dist_FL);
        int counts_FR = (int) (COUNTS_PER_INCH * dist_FR);
        int counts_RL = (int) (COUNTS_PER_INCH * dist_RL);
        int counts_RR = (int) (COUNTS_PER_INCH * dist_RR);


        frontRightMotor.setTargetPosition(counts_FR);
        frontLeftMotor.setTargetPosition(counts_FL);
        rearLeftMotor.setTargetPosition(counts_RL);
        rearRightMotor.setTargetPosition(counts_RR);
